...
describe("server.js", () => {
  ...
  describe("GET /", () => {
    ...
    //--------------------------------------------------------------------//
    beforeEach(async () => {
      res = await request(server).get("/"); //in this case we can omit this line in a tests
    });

    it("should return 200 OK in async/await way", async () => {
      // const res = await request(server).get("/"); //request(server) will listen his own port
      //console.log(res);
    ...
    it("should be json", async () => {
      const res = await request(server).get("/"); //new instancs of a server
